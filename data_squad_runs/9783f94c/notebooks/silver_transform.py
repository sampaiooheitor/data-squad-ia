...
# Schema casting
# No casts needed — all columns are string type

# DQ filtering
error_cond = F.col("`1pIrYwUzHyvf3c7ataAQeskI_hnhlSgEO`").isNotNull()
df_clean    = df.filter(error_cond)
df_rejected = df.filter(~error_cond).withColumn("_dq_error", F.lit("DQ rule failed: 1pIrYwUzHyvf3c7ataAQeskI_hnhlSgEO not_null"))
